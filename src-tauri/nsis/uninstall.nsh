...
!macro NSIS_HOOK_PREUNINSTALL
  DetailPrint "Stopping Cpaste before uninstall..."

  # 应用关闭主窗口时会缩到托盘，卸载器不能依赖普通的关闭请求。
  # 这里在开始删除文件前强制结束已安装的进程，避免 exe 被占用。
  nsExec::ExecToLog '"$SYSDIR\taskkill.exe" /F /T /IM Cpaste.exe'
  nsExec::ExecToLog '"$SYSDIR\taskkill.exe" /F /T /IM cpaste-app.exe'
  Sleep 1200

  # 先清理常见的自启动注册表项，避免系统在卸载后继续拉起已删除的程序。
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "Cpaste"
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "cpaste"
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "cpaste-app"
!macroend
...
!macro NSIS_HOOK_POSTUNINSTALL
  ...
  DeleteRegKey HKCU "Software\cpaste\Cpaste"
  DeleteRegKey /ifempty HKCU "Software\cpaste"
  ...
  nsExec::Exec '"$SYSDIR\cmd.exe" /C start "" /MIN powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command "Start-Sleep -Seconds 3; Stop-Process -Name Cpaste,cpaste-app -Force -ErrorAction SilentlyContinue; Remove-Item -LiteralPath ''$INSTDIR'' -Force -Recurse -ErrorAction SilentlyContinue"'
!macroend
