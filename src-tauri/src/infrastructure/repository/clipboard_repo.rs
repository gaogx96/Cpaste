...
// is_syncable_content_type removed (cloud sync was deleted)
...
impl SqliteClipboardRepository {
    ...
    // upsert_tombstone_with_conn and clear_tombstone_with_conn removed
    // (cloud sync was deleted)
    ...
    pub fn delete_with_conn(
        ...
        if let Some((_content_type, _content_hash)) = tombstone {
        }
    ...
    /// Load entries by their IDs (for on-the-fly group matching).
    pub fn get_entries_by_ids_with_conn(
        &self,
        conn: &Connection,
        ids: &[i64],
    ) -> Result<Vec<ClipboardEntry>, String> {
        ...
        let sql = format!(
            "SELECT id, content_type, content, html_content, source_app, timestamp, preview,
                    ...
                    smart_group_id, smart_group_name, note, group_confidence, group_reason,
                    group_match_type, group_manual_override
             FROM clipboard_history WHERE id IN ({})",
            placeholders.join(",")
        );
        ...
        let rows = stmt.query_map(rusqlite::params_from_iter(ids.iter()), map_row)
            .map_err(|e| e.to_string())?;
        let mut entries = Vec::new();
        for row in rows {
            entries.push(row.map_err(|e| e.to_string())?);
