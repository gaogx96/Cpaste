...
use crate::app_state::{
    AppDataDir, PasteQueue, SessionHistory, SettingsState,
};
...
pub fn init(app: &mut App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    crate::logger::init(app_dir.join("cpaste.log"));
    info!(">>> [STARTUP] Cpaste starting up...");
    ...
    let conn = database::init_db(&db_path_str).map_err(|e| {
        ...
        WindowExt::show_error_box("Cpaste 启动错误", &err_msg);
        e
    })?;
        ...
        if let Ok(entries) = std::fs::read_dir(&temp_dir) {
            for entry in entries.flatten() {
                if let Ok(name) = entry.file_name().into_string() {
                    if name.starts_with("Cpaste_Clip_") {
...
fn setup_tray(app: &App, hide_tray: bool) {
    ...
    let quit_i = MenuItem::with_id(app, "quit", "退出 Cpaste", true, None::<&str>).unwrap();
...
pub fn handle_global_shortcut(app: &AppHandle, shortcut: &tauri_plugin_global_shortcut::Shortcut) {
    ...
    if let Ok(seq_s) = {
        ...
        if shortcut == &seq_s {
            ...
            let has_items = {
                let q = app.state::<crate::app_state::PasteQueue>();
                let guard = q.0.lock().unwrap();
