...
pub fn toggle_autostart(enabled: bool) -> AppResult<()> {
    ...
    if enabled {
        key.set_value("Cpaste", &cmd)
            .map_err(|e| AppError::Internal(e.to_string()))?;
    } else {
        let _ = key.delete_value("Cpaste");
        let _ = key.delete_value("cpaste");
    }
    Ok(())
}
...
pub fn is_autostart_enabled() -> AppResult<bool> {
    ...
    let key = hkcu
        ...
        .map_err(|e| AppError::Internal(e.to_string()))?;
    Ok(key.get_value::<String, _>("Cpaste").is_ok() || key.get_value::<String, _>("cpaste").is_ok())
}
...
fn rewrite_content_path(
    ...
    if value.starts_with(old_prefix) {
        return Some(format!("{}{}", new_prefix, &value[old_prefix.len()..]));
    }
    if value.starts_with(old_prefix_slash) {
        return Some(format!(
            "{}{}",
            new_prefix_slash,
            &value[old_prefix_slash.len()..]
        ));
    }
    None
}
...
fn rewrite_html_paths(
    ...
    let mut updated = value.replace(old_prefix, new_prefix);
