...
pub(crate) fn remember_recent_paste(
    ...
    if let Some(queue) = app_handle.try_state::<crate::app_state::PasteQueue>() {
        ...
        let mut guard = queue.0.lock().unwrap();
        // If this paste matches the last pasted item, don't add it to queue
        if guard.last_action_was_paste
            && guard.last_pasted_fingerprint.as_deref() == Some(&fingerprint)
        {
            return;
        }
        guard.last_action_was_paste = false;
        guard.last_pasted_fingerprint = Some(fingerprint.clone());
        guard.last_pasted_content = Some(content.to_string());
    }
}
