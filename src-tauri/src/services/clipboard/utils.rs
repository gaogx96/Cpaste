...
pub const RICH_IMAGE_FALLBACK_PREFIX: &str = "<!--CPASTE_RICH_IMAGE:";
pub const RICH_IMAGE_FALLBACK_SUFFIX: &str = "-->";
pub const RICH_NAMED_FORMATS_PREFIX: &str = "<!--CPASTE_RICH_FORMATS:";
pub const RICH_NAMED_FORMATS_SUFFIX: &str = "-->";
// REMOTE_IMAGE_MAX_BYTES and REMOTE_IMAGE_TIMEOUT_SECS removed
// along with fetch_remote_image
...
// fetch_remote_image removed — downloading remote images on copy
// would leak the user's IP to third-party servers. Remote image URLs
// are preserved as-is in the clipboard entry instead.
...
mod tests {
    ...
    fn create_test_png_file(name: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let dir =
            std::env::temp_dir().join(format!("cpaste_clip_utils_{}_{}", std::process::id(), unique));
