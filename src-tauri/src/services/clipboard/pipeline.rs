...
impl PipelineStage for TransformationStage {
    fn process(&self, ctx: &mut PipelineContext) {
        ...
        // Sensitive Info tagging has been removed (privacy protection fields removed)
...
impl PipelineStage for ValidationStage {
    fn process(&self, ctx: &mut PipelineContext) {
        ...
        // Recent paste echo check has been removed (PasteQueue functionality removed)
...
impl PipelineStage for DistributionStage {
    fn process(&self, ctx: &mut PipelineContext) {
        ...
        // Sequential paste queue: add to queue when sequential mode is on
        let settings_seq = ctx.app_handle.state::<crate::app_state::SettingsState>();
        if settings_seq.sequential_mode.load(Ordering::Relaxed) {
            if let Some(entry) = &ctx.entry {
                let queue = ctx.app_handle.state::<crate::app_state::PasteQueue>();
                let mut guard = queue.0.lock().unwrap();
                // Reset queue on new copy
                guard.items.clear();
                guard.items.push_back(entry.id);
